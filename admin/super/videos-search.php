...
<body>
    ...
    <div id="searchbuttonContainer">
        <div id="searchbuttonContent">
            <form action="videos-search.php" method="post" class="searchForm">
    ...
    <div id="tableContainer">
        <div id="tableContent">
            <table class="table-videos">
                ...
                <tbody>
                    <?php
                        $query = mysqli_query($con, "SELECT * FROM videos WHERE title LIKE '%".$search."%' AND deleted='0' AND disapproved='0' || filename LIKE '%".$search."%' AND deleted='0' AND disapproved='0' || filetype LIKE '%".$search."%' AND deleted='0' AND disapproved='0' || source LIKE '%".$search."%' AND deleted='0' AND disapproved='0' || uploadedby LIKE '%".$search."%' AND deleted='0' AND disapproved='0' ORDER BY id ASC");
                        ...
                        if($check != 0){
                            while($row = mysqli_fetch_assoc($query)){
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="videos-edit.php" method="post">
                                            <button class="vid-button" id="videoedit" name="videoedit"><input type="hidden" name="vidid" id="vidid" value="<?php echo $row["id"] ?>"><img src="/videostreamingportal/icons/edit.png" class="vid-buttonicon"></button>
                                        </form>
                                    </td>
                                    <td>
                                        <?php
                                            if($row["disabled"] == "0"){
                                                ...
                                                <form action="" method="post">
                                                    <button class="vid-button" id="videodisable" name="videodisable"><input type="hidden" name="vidid" id="vidid" value="<?php echo $row["id"] ?>"><input type="hidden" name="vidtitle" id="vidtitle" value="<?php echo $row["title"] ?>"><img src="/videostreamingportal/icons/disable.png" class="vid-buttonicon"></button>
                                                </form>
                                                <?php
                                            }
                                            else{
                                                ?>
                                                <form action="" method="post">
                                                    <button class="vid-button" id="videoenable" name="videoenable"><input type="hidden" name="vidid" id="vidid" value="<?php echo $row["id"] ?>"><input type="hidden" name="vidtitle" id="vidtitle" value="<?php echo $row["title"] ?>"><img src="/videostreamingportal/icons/enable.png" class="vid-buttonicon"></button>
                                                </form>
                                                <?php
                                            }
                                        ?>
                                    ...
                                    <td>
                                        <form action="" method="post">
                                            <button class="vid-button" id="videodelete" name="videodelete"><input type="hidden" name="vidid" id="vidid" value="<?php echo $row["id"] ?>"><input type="hidden" name="vidtitle" id="vidtitle" value="<?php echo $row["title"] ?>"><img src="/videostreamingportal/icons/binwhite.png" class="vid-buttonicon"></button>
                                        </form>
                                    </td>
                                </tr>
                                <?php
    ...
    <?php
        if(isset($_POST["videodisable"])){
            ...
            ?>
                <script>
                    alert("The views for this video is now disabled!");
            ...
            $date = date('Y-m-d');
            $actor = $_SESSION['session_adminname'];
            $action = "DISABLED VIEW FOR " . $title;
            mysqli_query($con, "INSERT INTO history (actor, action, dateofaction) VALUES ('$actor', '$action', '$date')");
        }
    ?>
    <?php
        if(isset($_POST["videoenable"])){
            $title = $_POST["vidtitle"];
            $id = $_POST["vidid"];
            ...
            ?>
                <script>
                    alert("The views for this video is now enabled!");
                    window.location.replace(window.location.pathname + window.location.search + window.location.hash);
                </script>
            <?php
    ...
    <?php
        if(isset($_POST["videodelete"])){
            ...
            ?>
                <script>
                    alert("The video is moved to recycle bin!");
                    window.location.replace(window.location.pathname + window.location.search + window.location.hash);
                </script>
            <?php
            ...
            $date = date('Y-m-d');
            $actor = $_SESSION['session_adminname'];
            $action = "MOVED " . $title . " TO RECYCLE BIN";
            mysqli_query($con, "INSERT INTO history (actor, action, dateofaction) VALUES ('$actor', '$action', '$date')");
        }
    ?>
