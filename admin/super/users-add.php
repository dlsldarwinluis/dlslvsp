...
<body>
        ...
        <div class="form-container">
            ...
            <form action="" method="post" id="addvideoForm">
                <div class="videoform-div">
                    <input type="text" id="userfullname" name="userfullname" class="videoform-input" placeholder="Fullname" required>
                </div>
                <div class="videoform-div">
                    <input type="email" id="useremail" name="useremail" class="videoform-input" placeholder="Email" required>
                </div>
                <div class="videoform-div">
                    <select name="userspecification" id="userspecification" class="videoform-select" required>
                ...
                <div class="videoform-div">
                    <input type="text" id="userpassword" name="userpassword" class="videoform-input" value="dlslpass123" placeholder="Default Password" required>
                </div>
                <div class="videoform-div">
    ...
    <?php
        if(isset($_POST['vidsubmit'])){
            ...
            $userspecification = $_POST["userspecification"];
            $userpassword = $_POST["userpassword"];
            $userdeleted = "0";
            $userverified = "1";

            $query = mysqli_query($con, "SELECT * FROM users WHERE fullname='$userfullname'");
            $check = mysqli_num_rows($query);

            if($check == 0){
                mysqli_query($con, "INSERT INTO users (fullname, specification, email, password, deleted, verified) VALUES ('$userfullname', '$userspecification', '$useremail', '$userpassword', '$userdeleted', '$userverified')");

                ?>
                <script>
                    alert("New user successfully added!");
                    location.reload();
                    window.location = "/videostreamingportal/admin/super/users-list.php";
                </script>
                <?php
            }
            else{
                ?>
                <script>
                    alert("User account already existing!");
                    location.reload();
                    window.location = "/videostreamingportal/admin/super/users-list.php";
                </script>
                <?php
